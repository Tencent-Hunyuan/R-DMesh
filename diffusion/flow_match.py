...
class FlowMatchScheduler():
    ...
    def step(self, model_output, timestep, sample, to_final=False, **kwargs):
        ...
        prev_sample = sample + model_output * (sigma_ - sigma).to(model_output.device)
        return prev_sample
    ...
    def add_noise(self, original_samples, noise, timestep):
        ...
        if len(timestep.shape) > 0:
            ...
            timestep_id = torch.argmin((self.timesteps - timestep).abs())
        sigma = self.sigmas[timestep_id]
        # 修正：确保 sigma shape 可广播到 original_samples/noise，并在同一 device
    ...
    def training_weight(self, timestep):
        # 保证 self.timesteps 和 timestep 在同一设备
        ...
        # 保证索引和权重都在 cpu
        weights = self.linear_timesteps_weights.cpu()[timestep_id.cpu()]
        return weights
